...
class breedModel {
    ...
    public function newBreed($user_id, $scenario, $name) {
        ...
        if(!$stmt->execute(array($user_id, $scenario, $name))) {
            ...
            // Ziel-Käfig erstellen
            $newFinishedCageId = $this->newCage(20, $newBreedId);
            if ($newFinishedCageId < 0) return Array('id' => -1, 'msg' => "Ziel-Käfig konnten nicht erstellt werden.");
            ...
            if ($scenario == "easy_1") {
                ...
                // Mäuse erzeugen
                // weiblich
                $newMouseId_f_1 = $this->newMouse($newCageId_1, $newBreedId, $user_id, 1, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_f_2 = $this->newMouse($newCageId_1, $newBreedId, $user_id, 1, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_f_3 = $this->newMouse($newCageId_1, $newBreedId, $user_id, 1, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                ...
                // männlich
                $newMouseId_m_1 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "BB", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_m_2 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "BB", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_m_3 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "BB", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                ...
                // Mäuse erzeugen
                // weiblich
                $newMouseId_f_1 = $this->newMouse($newCageId_1, $newBreedId, $user_id, 1, "AB", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_f_2 = $this->newMouse($newCageId_1, $newBreedId, $user_id, 1, "AB", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                ...
                // männlich
                $newMouseId_m_1 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_m_2 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                $newMouseId_m_3 = $this->newMouse($newCageId_2, $newBreedId, $user_id, 0, "AA", $initWeight, NULL, NULL, NULL, 21, "-")['id'];
                if ($newMouseId_m_1 < 0 || $newMouseId_m_2 < 0 || $newMouseId_m_3 < 0) {
                    $this->deleteBreed($newBreedId);
                    return Array('id' => -1, 'msg' => "M?use konnten nicht erstellt werden.");
                }
    ...
    public function saveBreed($breed) {
        ...
        foreach($breed['cages'] as $cage) {
            foreach($cage['mice'] as $mouse) {
                $stmt =     "UPDATE mouse ".
                            "SET cage_id = ?, weight = ?, age = ?, user_gender = ?, pregnant=?, mating_id=? ".
                            "WHERE id = ?";
                $stmt = $this->db->prepare($stmt);

                if(!$stmt->execute(array($mouse['cage_id'], $mouse['weight'], $mouse['age'], $mouse['user_gender'], $mouse['pregnant'], $mouse['mating_id'], $mouse['id']))) {
                    return false;
                }
            }
        }
